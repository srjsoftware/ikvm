...
namespace IKVM.Internal
{
    ...
    abstract class Annotation
    {
        ...
        {
            if (def[0].Equals(AnnotationDefaultAttribute.TAG_ANNOTATION) == false)
            ...
            if (ClassFile.IsValidFieldSig(annotationClass))
            {
                var tw = owner.GetClassLoader().RetTypeWrapperFromSig(annotationClass.Replace('/', '.'), LoadMode.Link);
        ...
        static object LookupEnumValue(Type enumType, string value)
        {
            var field = enumType.GetField(value, BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
            if (field != null)
            ...
            if (targetType.IsEnum)
            {
                ...
                if (((object[])obj)[0].Equals(AnnotationDefaultAttribute.TAG_ARRAY))
                {
                    ...
                    for (int i = 1; i < arr.Length; i++)
                    {
                        ...
                        var newval = LookupEnumValue(targetType, s);
                        if (value == null)
                            value = newval;
                        else
        ...
        {
            var ci = type.GetConstructor(new Type[] { typeof(SecurityAction) });
            if (ci == null)
            {
                    ...
                    if (def[1].Equals("Ljava/lang/annotation/Retention;"))
                    {
                        for (int i = 2; i < def.Length; i += 2)
                        {
                            if (def[i].Equals("value"))
                            {
                                ...
                                if (val != null && val.Length == 3 && val[0].Equals(AnnotationDefaultAttribute.TAG_ENUM) && val[1].Equals("Ljava/lang/annotation/RetentionPolicy;") && val[2].Equals("RUNTIME"))
                                {
                                    return true;
                                }
                            }
        ...
        {
            var copy = false;
            var def = (object[])annotation;
            for (int i = 3; i < def.Length; i += 2)
            {
                var val = def[i] as object[];
                if (val != null)
                {
                    var newval = ValueQualifyClassNames(loader, val);
                    if (newval != val)
                    {
                        if (!copy)
                        {
                            copy = true;
                            var newdef = new object[def.Length];
        ...
        private static object[] ValueQualifyClassNames(ClassLoaderWrapper loader, object[] val)
        {
            ...
            else if (val[0].Equals(AnnotationDefaultAttribute.TAG_CLASS))
            {
                ...
                if (sig.StartsWith("L"))
                {
                    var tw = loader.LoadClassByDottedNameFast(sig.Substring(1, sig.Length - 2).Replace('/', '.'));
                    if (tw != null)
